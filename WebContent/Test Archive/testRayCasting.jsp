...
// 		var toolGeometry = new THREE.CubeGeometry( 0.7, 5, 0.7);
		...
		for (xCoord = 0; xCoord <= 5; xCoord += elemSize){
			for (yCoord = 0; yCoord <= 5; yCoord += elemSize){
				var billetGeometry = new THREE.CubeGeometry( elemSize, elemSize, 5 );
				var billetMaterial = new THREE.MeshPhongMaterial( { color: 0x0000ff, wireframe: false } );
				var elem = new THREE.Mesh( billetGeometry, billetMaterial );
				elem.position.x = xCoord;
				elem.position.y = yCoord;
				elem.position.z = 0;
				scene.add ( elem );
				collidableMeshList.push(elem);
			}
		}
		...
		var raycasterOrigin = tool.position.clone();
		var raycaster = new THREE.Raycaster(tool.position.clone(), raycasterDirection, 0, 5);
		var raycasterOffsetY = 2.5;
		function animate() {
			if (machining){
				if (counter <= 0){
					i=0.05;
...
// 			raycaster.ray.origin.x = tool.position.x;
// 			raycaster.ray.origin.z = tool.position.z;
			
			raycaster.set(tool.position.clone(), raycasterDirection);
			...
			if (collisionResults.length > 0){
				...
				collisionResults[0].object.material.color.setHex( 0x0ff0f0 );
			}
 			...
 			document.getElementById('programs').value = collisionResults.length;
